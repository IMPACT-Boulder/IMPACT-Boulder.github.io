...
const DataInputControl: React.FC<DataInputControlProps> = ({ onDataUpdate }) => {
  ...
  //     try {
  //       const response = await fetch('http://127.0.0.1:5000/api/dust_types');
  //       const data: { dust_name: string }[] = await response.json();
  //       setDustTypeOptions(data.map((item) => item.dust_name));
  //     } catch (error) {
  ...
  //       const data: { group_name: string }[] = await response.json();
  //       const sortedGroupNames = data.map((item) => item.group_name).sort();
