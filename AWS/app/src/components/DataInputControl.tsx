...
const DataInputControl: React.FC<DataInputControlProps> = ({ onDataUpdate }) => {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      const apiUrl = `https://10.247.29.177:3000/api/data?limit=${formState.limitValue}&velocityLow=${
        formState.velLow * 1000
      }&velocityHigh=${formState.velHigh * 1000}&qualityLow=${formState.qualLow}&qualityHigh=${formState.qualHigh}&massLow=${
        formState.massLow
      }&massHigh=${formState.massHigh}&chargeLow=${formState.chargeLow}&chargeHigh=${formState.chargeHigh}&radiusLow=${
        formState.radiusLow
      }&radiusHigh=${formState.radiusHigh}&timeLow=${formState.timeLow}&timeHigh=${formState.timeHigh}&dustType=${dustTypesParam}&groupName=${groupNamesParam}&tagName=${tagNamesParam}`;
